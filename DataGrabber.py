...
class ExtractIntermediateOutputs(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        if epoch in self.epochs_to_extract:
            Path(f"{directory_name}/{self.sweep_run}/epoch{epoch}/").mkdir(parents=True, exist_ok=True)
            for layer in range(3):
                ...
                with open(f"{directory_name}/{self.sweep_run}/epoch{epoch}/layer{layer}.npy", "wb") as datafile:
